...
在以太坊中，燃气优化是指重写Solidity代码，以在以太坊虚拟机（EVM）中消耗更少的燃气单位，同时实现相同的业务逻辑。

这篇文章超过11,000个字，不包括源代码，是关于燃气优化的最完整的资料。

要完全理解本教程中的技巧，您需要了解EVM的工作原理，您可以通过参加我们的[燃气优化课程](https://hackmd.io/eQJUW4PLQN-6HRrgyxdhsQ?view)，[Yul课程](https://hackmd.io/eQJUW4PLQN-6HRrgyxdhsQ?view)，以及练习[Huff Puzzles](https://github.com/RareSkills/huff-puzzles)来学习。

然而，如果您只想知道代码中可能进行燃气优化的部分，本文提供了很多可以参考的领域。

## 燃气优化技巧并非总是有效

某些燃气优化技巧只在特定情况下有效。例如，直观上，以下代码：
...
效率更低，因为在条件取反时会消耗额外的操作码。令人意外的是，有很多情况下，这种优化实际上会增加交易的成本。Solidity编译器有时是不可预测的。

因此，在选择特定算法之前，您应该实际测量替代方案的效果。将其中一些技巧视为引起编译器可能出人意料的领域的注意。

本文档中标记为非通用的一些技巧。燃气优化技巧有时取决于编译器在本地的操作。通常应同时测试代码的最优版本和非最优版本，以查看是否真正获得了改进。我们将记录一些令人惊讶的情况，即本应导致优化的情况实际上导致了更高的成本。

其次，当使用Solidity编译器的--via-ir选项时，某些优化行为可能会发生变化。

## 注意复杂性和可读性

燃气优化通常会使代码变得更难读和更复杂。一个好的工程师必须在主观上权衡哪些优化是值得的，哪些不是。

## 这里无法对每个主题进行全面处理

我们无法详细解释每个优化，也没有必要这样做，因为有其他在线资源可供参考。例如，对于第二层和状态通道的完整或大部分处理将超出范围，而且有其他在线资源可以详细学习这些主题。

本文的目的是提供最全面的技巧列表。如果某个技巧感觉陌生，可以作为进一步自学的提示。如果标题看起来是你已经了解的技巧，可以简单浏览该部分。

## 我们不讨论特定应用的技巧

例如，有一种高效的方法可以确定一个数字是否为质数，但这种情况非常罕见，将空间用于此将降低本文的价值。同样，在我们的[Tornado Cash教程](https://www.rareskills.io/post/how-does-tornado-cash-work)中，我们建议了提高代码库效率的方法，但在这里包含该内容对读者没有好处，因为它过于特定于应用。
...
当存储变量从零变为非零时，用户必须支付总共22,100 gas（20,000 gas用于从零到非零的写入，2,100 gas用于冷存储访问）。

这就是为什么Openzeppelin的重入保护使用1和2来注册函数的活动状态，而不是0和1。将存储变量从非零更改为非零只需花费5,000 gas。

## 2. 缓存存储变量：仅写入和读取存储变量一次

在高效的Solidity代码中，您经常会看到以下模式。从存储变量读取至少需要100 gas，因为Solidity不会缓存存储读取。写入要昂贵得多。因此，您应该手动缓存变量，以便仅进行一次存储读取和一次存储写入。
...
将相关变量打包到同一个槽位中可以通过最小化昂贵的存储相关操作来减少燃气成本。
...
像打包相关状态变量一样，打包结构体成员可以帮助节省燃气。（需要注意的是，在 Solidity 中，结构体成员按顺序存储在合约的存储中，从它们初始化的槽位位置开始）。
...
我们可以通过打包结构体成员来减少上面示例的燃气消耗，如下所示。
...
如果长度为32字节或更长，它们定义的槽位中存储的是字符串长度 * 2 + 1，而实际数据存储在其他位置（该槽位的 keccak 哈希值）。
...
如果我们将字符串的十六进制值（长度超过32字节）连接起来，不包括长度，我们可以将其转换回原始字符串（使用Python）。
...
这样可以节省大量的gas，因为我们不进行任何昂贵的存储读取操作。

## 7. 使用映射而不是数组来避免长度检查

当存储您希望按特定顺序组织并使用固定键/索引检索的项目列表或组时，通常使用数组数据结构是常见的做法。这种方法很有效，但您知道可以实现一个技巧，每次读取时可以节省2000多个gas吗？
...
仅仅通过使用映射，我们就可以节省2102个gas。为什么？在底层，当您读取数组的索引值时，Solidity会添加字节码来检查您是否正在读取有效的索引（即索引严格小于数组的长度），否则会回滚并显示恐慌错误（具体为Panic(0x32)）。这样可以防止读取未分配或更糟糕的已分配存储/内存位置。

由于映射的方式是（简单的键=>值对），不需要进行这样的检查，我们可以直接从存储槽中读取。重要的是要注意，当以这种方式使用映射时，您的代码应确保不要读取超出规范数组索引的位置。
...
写入（SSTORE）和读取（SLOAD）的成本在燃气消耗方面非常昂贵。写入 32 字节的成本为 22,100 燃气，相当于每字节约 690 燃气。另一方面，写入智能合约的字节码的成本为每字节 200 燃气。
...
1. 首先将我们的数据复制到内存中，因为 EVM 会从内存中获取这些数据并将其存储为运行时代码。了解更多信息，请查看此处。
...
这种设计选择使我们能够通过仅提供盐值（可以少于 20 字节）高效地计算出数据的指针地址。从而使我们能够将指针与其他变量一起打包，从而降低存储成本。
...
- 然后，在接收到指针地址后，使用相同的EXTCODECOPY操作码来获取所需的数据。
...
注意：在使用存储指针时，务必小心不要引用[悬空指针](https://docs.soliditylang.org/en/v0.8.21/types.html#dangling-references-to-storage-array-elements)。（这是RareSkills的一位讲师制作的关于悬空指针的[视频教程](https://www.youtube.com/watch?v=Zi4BANKFNP8)）。
...
当将存储变量设置为零时，会获得退款，因此如果存储变量的最终状态为零，则计数所花费的净燃气将更少。
